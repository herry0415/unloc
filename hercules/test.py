...
# from data.snail import Snail # todo 需要切换lidar数据集
from data.snail_radar import Snail # todo 需要切换lidar数据集
from util import config
from model.raloc import RALoc
...
SEQUENCE_NAME = 'if' #['if' 'iaf']  #['Library', 'Mountain', 'Sports']
sensor_type = 'radar'  #todo ['lidar','radar'] 
SUB_SEQUENCE_NAME =  '20240123_3' # Library_03_Day  Complex_03_Day
# if   ['20240116_eve_5', '20240116_5', '20240123_3']  # test 集
# iaf  [ '20240113_3', '20240116_eve_4', '20231201_3']  # test 集'20231213_3',
def get_parser():
    parser = argparse.ArgumentParser(description='PyTorch Point Cloud Localization')  # hercules_radar
    parser.add_argument('--config', type=str, default='config/4DRadarLoc/snail_radar.yaml',
                        help='config file')#todo yaml文件修改路径 ldq/code/RALoc/  hercules_radar
...
def main(args):
    ...
    if args.data_name == 'NCLT':
       test_pt_dataset = NCLT(args.data_root,split='test')
    elif args.data_name == 'QEOxford':
       test_pt_dataset = QEOxford(args.data_root,split='test')
    elif args.data_name == 'Oxford':
       test_pt_dataset = Oxford(args.data_root,split='test')  
    elif args.data_name == 'Snail':    # todo 可以加新的类别
       test_pt_dataset = Snail(args.data_root, split='test')
    elif args.data_name == 'Hercules':
       test_pt_dataset = Hercules(args.data_root,split='test') #todo 加上测试数据集说明
...
def valid_one_epoch(model, ransac, test_loader, test_writer, lenset, device, threshold):
    ...
    for step, input_dict in enumerate(tqdm_loader):
        ...
        input_dict['ds_xyz'] = input_dict['ds_xyz'].to(device, dtype=torch.float32)
        ...
        start = time.time()
        pred_shift, labels, coors,mat = run_model(model, input_dict, True)
        # pred_shift, labels, coors = run_model(model, input_dict, True)
    ...
    # image_filename = os.path.join(os.path.expanduser(args.save_path), f'epoch_{args.resume_epoch}_lidar_{SUB_SEQUENCE_NAME}_{SEQUENCE_NAME}.png') #todo 注意改radar/lidar
    subfolder_name = f'epoch_{args["resume_epoch"]}_{SUB_SEQUENCE_NAME}_{SEQUENCE_NAME}'
    save_subdir = os.path.join(args.save_path, subfolder_name)
    image_filename = os.path.join(os.path.expanduser(save_subdir), f'epoch_{args.resume_epoch}_{sensor_type}_{SUB_SEQUENCE_NAME}_{SEQUENCE_NAME}.png') #todo 注意改radar/lidar
    ...
    plt.xlabel('Data Num')
    plt.ylabel('Error (m)')
    image_filename = os.path.join(os.path.expanduser(save_subdir), '{:s}.png'.format('distribution_t_' + str(threshold)))
    fig.savefig(image_filename, dpi=200, bbox_inches='tight')
    ...
    image_filename = os.path.join(os.path.expanduser(save_subdir), '{:s}.png'.format('distribution_q_' + str(threshold)))
    fig.savefig(image_filename, dpi=200, bbox_inches='tight')

    # save error and trajectory 
    error_t_filename = osp.join(save_subdir, 'error_t.txt')
    error_q_filename = osp.join(save_subdir, 'error_q.txt')
    pred_q_filename = osp.join(save_subdir, 'pred_q.txt')
    pred_t_filename = osp.join(save_subdir, 'pred_t.txt')
    gt_t_filename = osp.join(save_subdir, 'gt_t.txt')
    np.savetxt(error_t_filename, error_t, fmt='%8.7f')
    np.savetxt(error_q_filename, error_q, fmt='%8.7f')
...
if __name__ == "__main__":
    ...
    #todo  从 resume 模型文件名中提取 epoch 失败的话就用时间戳
    if hasattr(args, 'resume') and args.resume is not None:
        ...
        if match:
            ...
            # 用时间戳字符串代替
            args.resume_epoch = time.strftime("%Y%m%d-%H%M%S", time.localtime())
    else:
        # 同样用时间戳字符串代替
        args.resume_epoch = time.strftime("%Y%m%d-%H%M%S", time.localtime())
    ...
    if args.data_name == 'QEOxford':
        LOG_FOUT = open(os.path.join(args.save_path, 'log2019-01-15-13-06-37.txt'), 'w')
        LOG_FOUT.write(str(args) + '\n')
        pose_m, pose_s = np.loadtxt('log/QEOxford/QEOxford_pose_stats.txt')
    elif args.data_name == 'NCLT':
        LOG_FOUT = open(os.path.join(args.save_path, 'log2012-05-26.txt'), 'w')
        LOG_FOUT.write(str(args) + '\n')
        pose_m, pose_s = np.loadtxt('log/NCLT/NCLT_pose_stats.txt')
    elif args.data_name == 'Hercules':
        subfolder_name = f'epoch_{args["resume_epoch"]}_{SUB_SEQUENCE_NAME}_{SEQUENCE_NAME}'
        if not os.path.exists(os.path.join(args.save_path, subfolder_name)):
            os.makedirs(os.path.join(args.save_path, subfolder_name))

        sequence_name = SEQUENCE_NAME #todo  在这里加新的类别
        pose_stats_file = os.path.join(args.data_root, sequence_name, sequence_name + '_' + 'lidar' + '_pose_stats.txt')
        pose_m, pose_s = np.loadtxt(pose_stats_file)
        LOG_FOUT = open(os.path.join(args.save_path, subfolder_name, f'epoch_{args["resume_epoch"]}_{SUB_SEQUENCE_NAME}_log.txt'), 'w')
        LOG_FOUT.write(str(args) + '\n')

    elif args.data_name == 'Snail':
        subfolder_name = f'epoch_{args["resume_epoch"]}_{SUB_SEQUENCE_NAME}_{SEQUENCE_NAME}'
        if not os.path.exists(os.path.join(args.save_path, subfolder_name)):
            os.makedirs(os.path.join(args.save_path, subfolder_name))
        sequence_name = SEQUENCE_NAME #todo  在这里加新的类别
        ...
        pose_stats_file = os.path.join(args.data_root, sequence_name, sequence_name + '_' + 'lidar' + '_pose_stats.txt')
        pose_m, pose_s = np.loadtxt(pose_stats_file)
        LOG_FOUT = open(os.path.join(args.save_path, subfolder_name, f'epoch_{args["resume_epoch"]}_{SUB_SEQUENCE_NAME}_log.txt'), 'w') #todo
        # LOG_FOUT = open(os.path.join(args.save_path, f'epoch_{args["resume_epoch"]}_log.txt'), 'w')
        # LOG_FOUT = open(os.path.join(args.save_path, f'epoch_{args["resume_epoch"]}_{SUB_SEQUENCE_NAME}_log.txt'), 'w') #todo
        LOG_FOUT.write(str(args) + '\n')
        pass
    # 占用2个核心
