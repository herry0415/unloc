...
def main(args):
    ...
    t_loss = nn.MSELoss().cuda()
    r_loss = nn.MSELoss().cuda()
    ...
    with torch.no_grad():
        ...
        for i_iter_val, data in enumerate(val_dataset_loader):
            ...
            Transgt = ((labels[:, 3:6] * posstd)+ posmean)
            Rotgt = labels[:, 0:3]
            ...
            r1 = r_loss(rot1,Rotgt.to(pytorch_device))
            t1 = t_loss(trans1, Transgt.to(pytorch_device))
            r2 = r_loss(rot2, Rotgt.to(pytorch_device))
            t2 = t_loss(trans2, Transgt.to(pytorch_device))
            ...
            r3 = r_loss(rot3, Rotgt.to(pytorch_device))
            t3 = t_loss(trans3, Transgt.to(pytorch_device))
            r4 = r_loss(rot4, Rotgt.to(pytorch_device))
            t4 = t_loss(trans4, Transgt.to(pytorch_device))
            r5 = r_loss(rot5, Rotgt.to(pytorch_device))
            t5 = t_loss(trans5, Transgt.to(pytorch_device))
            ...
            r6 = r_loss(rot6, Rotgt.to(pytorch_device))
            t6 = t_loss(trans6, Transgt.to(pytorch_device))
