...
class SlotAttentionAutoEncoder(nn.Module):
    ...
    def __init__(self, resolution=(32,32), num_slots=20, in_out_channels=512, iters=5):
        ...
        self.encoder_arch = [256, 'MP', 512, 'MP', 1024]
    ...
    def forward(self, image):
        ...
        # print(x.shape,"x encoder_cnn")
        x = einops.rearrange(x, 'b c h w -> b h w c')
        # print(x.shape, "x rearrange")
        x = self.encoder_pos(x)  # Position embedding.
        # print(x.shape, "x encoder_pos")
        x = spatial_flatten(x)  # Flatten spatial dimensions (treat image as set).
        # print(x.shape, "x spatial_flatten")
        ...
        # print(x.shape, "x mlp")
        # x = x.permute(0, 2, 1)
        # print(x.shape,"normal")

        # Slot Attention module.
        slots = self.slot_attention(x)
        slots = slots.permute(0,2,1)
        slots = self.mlp1(slots)
        # `slots` has shape: [batch_size, encoder_dims, 1] after mlp1
        # Reshape to [batch_size, encoder_dims] as 1024-dim feature vector
        return slots.reshape(slots.shape[0], slots.shape[1])
