...
class cylinder_dataset(data.Dataset):
    ...
    def pointclouddata(self,xyz):
        ...
        # print(return_xyz.shape, "return_xyz")
        return_fea = return_xyz
        data_tuple = (voxel_position, grid_ind, return_fea)
        return data_tuple

    def __getitem__(self, index):
        monoleft,monoright, monorear, radarimage, leftlidar, rightlidar,pose = self.point_cloud_dataset[index]
        lidareft_data_tuple =self.pointclouddata(leftlidar)
        data_tuple = (lidareft_data_tuple[0],lidareft_data_tuple[1],lidareft_data_tuple[2])
        lidaright_data_tuple = self.pointclouddata(rightlidar)
        data_tuple +=(lidaright_data_tuple[0],lidaright_data_tuple[1],lidaright_data_tuple[2])
        data_tuple +=  (monoleft,monoright, monorear, radarimage,pose)
        return data_tuple
...
def collate_fn_BEV(data):
    ...
    return torch.from_numpy(data2stackl), grid_indl,xyzl, torch.from_numpy(data2stackr), grid_indr,xyzr,monoleft,monoright,monorear,radarimage, point_label

#
# def collate_fn_BEV_test(data):
#     data2stack = np.stack([d[0] for d in data]).astype(np.float32)
#     label2stack = np.stack([d[1] for d in data]).astype(np.float32)
