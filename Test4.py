...
writepath = "/home/ibrahim/Desktop/run44"
import warnings
...
def main(args):
    ...
    with torch.no_grad():
        ...
        for i_iter_val, data in enumerate(val_dataset_loader):
            ...
            Transgt = torch.squeeze(((labels[:, 3:6] * posstd)+ posmean))
            Rotgt = torch.squeeze(labels[:, 0:3])
            ...
            r1 = r_loss(rot1.cpu().data.numpy(),np.array(Rotgt))
            t1 = t_loss(trans1.cpu().data.numpy(), np.array(Transgt))
            r2 = r_loss(rot2.cpu().data.numpy(), np.array(Rotgt))
            t2 = t_loss(trans2.cpu().data.numpy(), np.array(Transgt))
            #
            ...
            r3 = r_loss(rot3.cpu().data.numpy(), np.array(Rotgt))
            t3 = t_loss(trans3.cpu().data.numpy(), np.array(Transgt))
            r4 = r_loss(rot4.cpu().data.numpy(), np.array(Rotgt))
            t4 = t_loss(trans4.cpu().data.numpy(), np.array(Transgt))
            r5 = r_loss(rot5.cpu().data.numpy(), np.array(Rotgt))
            t5 = t_loss(trans5.cpu().data.numpy(), np.array(Transgt))
            ...
            r6 = r_loss(rot6.cpu().data.numpy(), np.array(Rotgt))
            t6 = t_loss(trans6.cpu().data.numpy(), np.array(Transgt))
            ...
            # r345 = ((r3 + r4 + r5) / 3)
            r345 = np.min([r3, r4, r5])
            R345 = (R345 +r345 )

            t345 = np.min([t3, t4, t5])
            T345 = (T345 + t345)
            ...
            # r136 = ((r1 + r3 + r6) / 3)
            r136 = np.min([r1, r3, r6])
            R136 = (R136 + r136 )

            # t136 = ((t1 + t3 + t6) / 3)
            t136 = np.min([t1, t3, t6])
            T136 = (T136 + t136)
            ...
            r236 = np.min([r2, r3, r6])
            R236 = (R236 + r236)

            # t236 = ((t2 + t3 + t6) / 3)
            t236 = np.min([t2, t3, t6])
            T236 = (T236 + t236)
            ...
            r123456 = np.min([r1, r2, r3,r4,r5, r6])
            R123456 = R123456 + r123456

            t123456 = np.min([t1, t2, t3, t4, t5, t6])
            ...
            r12345 = np.min([r1, r2, r3, r4, r5])
            R12345 = R12345 + r12345

            # t12345 = ((t2 + t1 +t3 + t4 + t5) / 5)
            t12345 = np.min([t1, t2, t3, t4, t5])
            ...
            # t3456 = ((t4 + t5 + t3 + t6) / 4)
            t3456 = np.min([t3, t4, t5, t6])
            r3456 = np.min([r3, r4, r5, r6])
            R3456 = R3456 + r3456
            T3456 = (T3456 + t3456)
            ...
            # t126 = ((t1 + t2 + t6) / 3)
            t126 = np.min([t1, t2, t6])

            r126 = np.min([r1, r2,  r6])
